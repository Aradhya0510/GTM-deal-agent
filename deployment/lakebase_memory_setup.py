...
spark.sql("DELETE FROM memory_ae_profiles WHERE ae_id IN ('ae-jamie@company.com', 'ae-sarah@company.com', 'ae-jamie', 'ae-sarah')")
...
context_inserts = [
    ("ACC-1001", "champion_change", "Sarah Chen was promoted to VP of IT Operations in March 2026 — she now has direct budget authority up to $2M", "ae-jamie", 0.95),
    ("ACC-1001", "competitor_mentioned", "BMC Helix had a demo with Meridian in March 2026 but feedback was negative — UI described as clunky", "ae-jamie", 0.92),
    ("ACC-1001", "budget_freeze", "Board mandate to reduce IT ops spend by 15% this fiscal year — champion sees platform consolidation as the path", "ae-jamie", 0.88),
    ("ACC-1002", "technical_requirement", "CISO Jennifer Walsh requires SOC team MTTR under 15 minutes — current average is 45 minutes", "ae-jamie", 0.90),
    ("ACC-1002", "competitor_mentioned", "Palo Alto XSOAR offering aggressive discounting on the Apex deal", "ae-jamie", 0.85),
    ("ACC-1002", "sentiment_shift", "Champion Michael Torres is pushing hard for AI-powered triage — his pet project with executive visibility", "ae-jamie", 0.87),
    ("ACC-1006", "champion_change", "James Liu (Head of Platform) went quiet in Feb 2026 — last known: building internal ROI business case, no budget approval from CEO", "ae-jamie", 0.82),
    ("ACC-1003", "timeline_shift", "CTO Amy Rodriguez wants to be live by August 2026 — fast timeline, no formal RFP yet", "ae-sarah", 0.91),
]
...
decisions = [
    ("OPP-3001", "ae-jamie", "Focus the executive briefing on 3-year TCO comparison vs current HP Service Manager + BMC stack", "accepted", "Good call — Kim is all about the numbers"),
    ("OPP-3002", "ae-jamie", "Lead with AI triage metrics in the email to CISO", "modified", "Good data but Jennifer is skeptical of AI claims — lead with the real incident detection story instead"),
    ("OPP-3006", "ae-jamie", "Offer a 20% discount to re-engage James Liu at Atlas Cloud", "rejected", "Too early for discounts — James needs help building the internal business case first, not a lower price"),
]
...
display(spark.sql("SELECT ae_id, email_style, avoid_competitors, raw_preferences FROM memory_ae_profiles WHERE ae_id = 'ae-jamie'"))
